 ...
 * one of the following choices <code> green, brown, or blue</code>.
 * </p>
 * <p>
 * The default value for color is <strong> <code>blue</code></strong>
 * </p>
 * <p>
 * The Conference chair shape has two shapes which is <strong>
 * <code>Rectangle, Oval</code></strong>
 * </p>
 * <p>
 * The default value for shape is <strong> <code>Oval</code></strong>
 * </p>
 * <p>
...
public class ConferenceChair extends Chair {
	...
	public ConferenceChair() {
		...
		// COMPLETE THIS
		super(15.0, "Oval", "blue");
        // if the default value are different with the superclass then use super( different default values inside);
	...
	public ConferenceChair(double chairManufCost, String chairShape, String chairColor, String chairCategory,
		...
		// COMPLETE THIS
		super(15.0, "Oval", "blue");
        // put default values first here,then the following will decide if there are new values coming in)
		if ((chairManufCost < 0) || (chairManufCost != 15.0)) {
		...
		Set<String> chaircolorset = new HashSet<String>(Arrays.asList("brown", "green", "blue"));
		if (!chaircolorset.contains(chairColor.toLowerCase())) {
			Chair.decrementNumberofChairCreated();
			throw new ChairColorNotValid("Invalid chair color.");
		}
		Set<String> chairshapeset = new HashSet<String>(Arrays.asList("rectangle", "oval"));
        ...
        // if there are new values coming in use the following to initalize it
		this.setChairColor(chairColor);
		this.setChairShape(chairShape);
		this.setChairManufCost(chairManufCost);
		// fields in superclass are private,if we want to use them, we use this.get method to access them
		// fields in superclass are private,if we want to initialize them in the subclass constructor
		// we need use this.set method to initalize them
	 ...
	 * object. The manufacturing cost depends on the Conference chair chairCategory
	 * field value and the color chair
	 * 
	 * <p>
	...
	public double getChairPrice() {
		...
		if (this.getChairColor().equalsIgnoreCase("blue")) {
			coloraddcost = 10.0;
		} else if (this.getChairColor().equalsIgnoreCase("brown")) {
			coloraddcost = 20.0;
		}
		...
		if (this.getChairCategory().equalsIgnoreCase("Basic")) {
			...
			return this.getChairManufCost() + coloraddcost + 55.0;
		} else if (this.getChairCategory().equalsIgnoreCase("Normal")) {
			return this.getChairManufCost() + coloraddcost + 75.0;
		} else
			return this.getChairManufCost() + coloraddcost + 90.0;
	}
	...
	public double getDeliveryCost(String nameofCity) {
		...
		if (!nameofCity.equalsIgnoreCase("Edmonton") && !nameofCity.equalsIgnoreCase("Winnipeg")
				&& !nameofCity.equalsIgnoreCase("Halifax")) {
			throw new IllegalArgumentException("The input city should be Edmonton, Winnipeg or Halifax  ");
		}

		if (nameofCity.equalsIgnoreCase("Edmonton")) {
			return (double) (55.0 + this.getChairPrice());

		} else if (nameofCity.equalsIgnoreCase("Winnipeg")) {
			return (double) (65.0 + this.getChairPrice());
		} else {
			return (double) (85.0 + this.getChairPrice());
	...
	private void checkChairNumberofWheels(int numberofWheels) throws IllegalArgumentException {
		...
		if (!(numberofWheels == 4) && !(numberofWheels == 6) && !(numberofWheels == 8)) {
			Chair.decrementNumberofChairCreated();
			throw new IllegalArgumentException(
					"the number of wheel of this Conference chair should be an even integer value between 4 and 8 ");
		}
	...
	private void checkChairCategory(String chairCategory) throws ChairCategoryNotValid {
		...
		if (!chairCategory.equalsIgnoreCase("Basic") && !chairCategory.equalsIgnoreCase("Normal")
				&& !chairCategory.equalsIgnoreCase("Delux")) {
			Chair.decrementNumberofChairCreated();
			throw new ChairCategoryNotValid("Chair chairCategory should be Basic, Normal or Delux ");
		}
